...
library(patchwork)
setwd("E:/lab/GEO/GSE25097/CA3-GSEA")
#输入文件
expFile = read.csv("exprSet_T.csv",header=TRUE,sep=",")
...
yd <- data.frame(y)
write.table(yd,file=paste0("GSE25097_GSEA-kegg-CA3.xls"),sep="\t",quote=F,col.names=T)
...
dev.off()
gseaplot2(Go_gseresult,45,color="red",pvalue_table = T)
...
write.table(af,file=paste0("GSE25097_gseKEGG-CA3.xls"),sep="\t",quote=F,col.names=T)
